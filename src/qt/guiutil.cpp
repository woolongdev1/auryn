...
void setupAddressWidget(QValidatedLineEdit* widget, QWidget* parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a auryn address (e.g. %1)").arg("S7VFR83SQbiezrW72hjcWJtcfip5krte2Z"));
...
{
    // return if URI is not valid or is no auryn: URI
...
{
    // Convert auryn:// to auryn:
    //
    //    Cannot handle this later, because auryn:// will cause Qt to see the part after // as host,
...
void openConfigfile()
{
    ...
    /* Open auryn.conf with the associated application */
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "auryn.lnk";
}
...
{
    return GetAutostartDir() / "auryn.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    if (!fAutoStart)
        ...
        if (!optionFile.good())
            return false;
        // Write a auryn.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=auryn\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the auryn app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if (fAutoStart && !foundItem) {
        // add auryn app to startup item list

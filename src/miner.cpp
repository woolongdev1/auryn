...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("aurynMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock)) {
        ...
        return error("aurynMiner : ProcessNewBlock, block not accepted");
    }
...
{
    LogPrint("debug", "aurynMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("auryn-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        CBlockIndex* pindexPrev = chainActive.Tip();
        if (!pindexPrev) {
            LogPrint("debug", "aurynMiner bailing, no pindexPrev\n");
            continue;
        }
        unique_ptr<CBlockTemplate> pblocktemplate(CreateNewBlockWithKey(reservekey, pwallet, fProofOfStake));
        if (!pblocktemplate.get()) {
            LogPrint("debug", "aurynMiner bailing, no pblocktemplate got\n");
            continue;
        } else {
            LogPrint("debug", "aurynMiner proceeding with pblocktemplate.\n");
        }
        CBlock* pblock = &pblocktemplate->block;
        IncrementExtraNonce(pblock, pindexPrev, nExtraNonce);

        LogPrint("debug", "aurynMiner skipping PoS section 4\n");
        //Stake miner main
        if (fProofOfStake) {
            LogPrint("debug", "aurynMiner : proof-of-stake block found %s \n", pblock->GetHash().ToString().c_str());
            if (pblock->IsZerocoinStake()) {
                ...
                if (!pwallet->GetZerocoinKey(bnSerial, key)) {
                    LogPrint("debug", "%s: failed to find zAURN with serial %s, unable to sign block\n", __func__, bnSerial.GetHex());
                    continue;
                }

                //Sign block with the zPIV key
                if (!SignBlockWithKey(*pblock, key)) {
                    LogPrint("debug", "aurynMiner(): Signing new block with zAURN key failed \n");
                    continue;
                }
            } else if (!SignBlock(*pblock, *pwallet)) {
                LogPrint("debug", "aurynMiner(): Signing new block with UTXO key failed \n");
                continue;
            }

            LogPrint("debug", "aurynMiner : proof-of-stake block was signed %s \n", pblock->GetHash().ToString().c_str());
        ...
        LogPrint("debug", "Running aurynMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
        ...
        LogPrint("debug", "Running aurynMiner with hashTarget %0x\n", hashTarget.GetCompact());
        while (true) {
...
void static ThreadBitcoinMiner(void* parg)
{
    ...
    try {
        ...
        LogPrint("debug", "ThreadaurynMiner() exception");
    }

    LogPrint("debug", "ThreadaurynMiner exiting\n");
}

...
#if defined(NDEBUG)
#error "auryn cannot be compiled without assertions."
#endif
...
unsigned int nStakeMinAge = 16 * 60;
...
bool ContextualCheckZerocoinSpendNoSerialCheck(const CTransaction& tx, const CoinSpend& spend, CBlockIndex* pindex, const uint256& hashBlock)
{
    ...
    if (pindex->nHeight >= Params().Zerocoin_Block_V2_Start()) {
        if (!spend.HasValidSignature())
            return error("%s: V2 zAURN spend does not have a valid signature", __func__);

        libzerocoin::SpendType expectedType = libzerocoin::SpendType::SPEND;
        if (tx.IsCoinStake())
            expectedType = libzerocoin::SpendType::STAKE;
        if (spend.getSpendType() != expectedType) {
            return error("%s: trying to spend zAURN without the correct spend type. txid=%s", __func__,
                         tx.GetHash().GetHex());
        }
    }
    ...
    if (pindex->nHeight > Params().Zerocoin_Block_EnforceSerialRange() &&
        ...
        return error("%s : zAURN spend with serial %s from tx %s is not in valid range\n", __func__,
...

    {
        ...
        if(tx.IsZerocoinSpend()){
            ...
            if (IsTransactionInChain(tx.GetHash(), nHeightTx))
                return state.Invalid(error("AcceptToMemoryPool : zAURN spend tx %s already in block %d",
...
int64_t GetBlockValue(int nHeight)
{
    ...
    //int64_t LAST_POW_BLOCK = 720;
    if (nHeight == 1) {
        CoinAmount = static_cast<int64_t>(3643000 * COIN);
    } else if ( nHeight > 1 && nHeight <= 250) {
        CoinAmount = static_cast<int64_t>(2000 * COIN);
    } else if ( nHeight > 250 && nHeight <= Params().LAST_POW_BLOCK()) {
        CoinAmount = static_cast<int64_t>(300 * COIN);
    } else if (nHeight > Params().LAST_POW_BLOCK() && nHeight <= 950) {
        CoinAmount = static_cast<int64_t>(28 * COIN);
    } else {
        CoinAmount = static_cast<int64_t>(0.5 * COIN);
    }
    return CoinAmount;
    }
    ...
    int64_t CoinAmount = 0;
    if (nHeight == 1) {
        CoinAmount = static_cast<int64_t>(3200000 * COIN);
    } else if ( nHeight > 1 && nHeight <= 600) {
        CoinAmount = static_cast<int64_t>(1000 * COIN);
    } else if ( nHeight > 600 && nHeight <= Params().LAST_POW_BLOCK()) {
        CoinAmount = static_cast<int64_t>(500 * COIN);
...
int64_t GetMasternodePayment(int nHeight, int64_t blockValue, int nMasternodeCount, bool isZPIVStake)
{
    ...
    if (nHeight <= 349) {
        ...
        //When zPIV is staked, masternode gets 1 AURN
        ret = 1 * COIN;
        if (isZPIVStake)
            ret = 1 * COIN;
    }
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck, bool fAlreadyChecked)
{
    ...
    */
    ...
    if (!UpdateZPIVSupply(block, pindex))
        return state.DoS(100, error("%s: Failed to calculate new zAURN supply for block=%s height=%d", __func__,
            ...
            for (const CTxIn txIn : tx.vin) {
                if (txIn.scriptSig.IsZerocoinSpend()) {
                    ...
                    if (count(vBlockSerials.begin(), vBlockSerials.end(), spend.getCoinSerialNumber()))
                        return state.DoS(100, error("%s : Double spending of zAURN serial %s in block\n Block: %s",
...
bool ContextualCheckZerocoinStake(int nHeight, CStakeInput* stake)
{
    if (nHeight < Params().Zerocoin_Block_V2_Start())
        return error("%s: zAURN stake block is less than allowed start height", __func__);

    if (CZPivStake* zPIV = dynamic_cast<CZPivStake*>(stake)) {
        CBlockIndex* pindexFrom = zPIV->GetIndexFrom();
        if (!pindexFrom)
            return error("%s: failed to get index associated with zAURN stake checksum", __func__);

        if (chainActive.Height() - pindexFrom->nHeight < Params().Zerocoin_RequiredStakeDepth())
            return error("%s: zAURN stake does not have required confirmation depth", __func__);
...
bool AcceptBlock(CBlock& block, CValidationState& state, CBlockIndex** ppindex, CDiskBlockPos* dbp, bool fAlreadyCheckedBlock)
{
	...
	isPoS = true;
        ...
        if (stake->IsZPIV() && !ContextualCheckZerocoinStake(pindexPrev->nHeight, stake.get()))
            return state.DoS(100, error("%s: staked zAURN fails context checks", __func__));
...
bool ProcessNewBlock(CValidationState& state, CNode* pfrom, CBlock* pblock, CDiskBlockPos* dbp)
{
    ...
    if (nMints || nSpends)
        LogPrintf("%s : block contains %d zAURN mints and %d zAURN spends\n", __func__, nMints, nSpends);
...
bool TestBlockValidity(CValidationState& state, const CBlock& block, CBlockIndex* const pindexPrev, bool fCheckPOW, bool fCheckMerkleRoot)
{
    ...
    if (!ContextualCheckBlock(block, state, pindexPrev))
        return false;
    LogPrint("debug", "Calling ConnectBlock with %s\n", block.ToString().c_str());
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
	...
	if (pfrom->cleanSubVer == "/auryn Core:1.7.3/" ||
            pfrom->cleanSubVer == "/auryn Core:1.7.2/" ||
            pfrom->cleanSubVer == "/auryn Core:1.7.1/" ||
            pfrom->cleanSubVer == "/auryn Core:1.7.0/" ||
            pfrom->cleanSubVer == "/auryn Core:1.6.3/" ||
            pfrom->cleanSubVer == "/auryn Core:1.6.2/" ||
            pfrom->cleanSubVer == "/auryn Core:1.6.1/" ||
            pfrom->cleanSubVer == "/auryn Core:1.6.0/" ||
            pfrom->cleanSubVer == "/auryn Core:1.5.7/" ||
            pfrom->cleanSubVer == "/auryn Core:1.2.1/" ||
            pfrom->cleanSubVer == "/auryn Core:1.2.0/" ||
            pfrom->cleanSubVer == "/auryn Core:1.0.1/") {
            LOCK(cs_main);
